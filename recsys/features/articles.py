...
def compute_features_articles(df: pl.DataFrame) -> pl.DataFrame:
    ...
    column_to_drop = ["detail_desc", "detail_desc_length"]
    existing_columns = df.columns
    columns_to_keep = [col for col in existing_columns if col not in column_to_drop]
...
def generate_embeddings_for_dataframe(
    ...
    Returns:
    pl.DataFrame: DataFrame with a new 'embeddings' column
    """
    ...
    total_rows = len(df)
    pbar = tqdm(total = total_rows , desc= "Generating embedding")
