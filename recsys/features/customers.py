...
class DatasetSampler:   
    ...
    def sample(
        ...
        """Samples a subset of customers and their corresponding transactions.

        This method first samples a specified number of customers from the
        `customers_df` DataFrame. It then filters the `transations_df` to
        include only the transactions made by the sampled customers.

        Args:
            customers_df: A Polars DataFrame containing customer data.
                Must include a "customer_id" column.
            transations_df: A Polars DataFrame containing transaction data.
                Must include a "customer_id" column.

        Returns:
            A dictionary with two keys:
            - "customers": A DataFrame with the sampled customers.
            - "transactions": A DataFrame with the transactions of the
              sampled customers.
        """
        random.seed(27)
